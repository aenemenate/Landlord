    ...
    public class SimplexNoise
    {
        ...
        public static float[,] Calc2D(int width, int height, float scale)
        {
            ...
            for (int i = 0; i < width; i++)
                ...
                {
                    values[i, j] = SumOctave(16, i, j, .5F, scale);
                }
            return values;
        }

        public static float SumOctave(int numOfIterations, int x, int y, float persistence, float scale)
        {
            float maxAmp = 0;
            float amp = 1;
            float freq = scale;
            ...
            {
                noise += Generate(x * freq, y * freq) * amp;
                maxAmp += amp;
                amp *= persistence;
                freq *= 2;
            }
